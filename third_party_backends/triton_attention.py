...
def bench_flash_attention(BATCH, H, N_CTX, D_HEAD, causal, dtype, device="cuda"):
    ...
    qk_matmul_flops = 2 * BATCH * H * N_CTX * N_CTX * D_HEAD
    pv_matmul_flops = 2 * BATCH * H * N_CTX * D_HEAD * N_CTX
    total_flops = qk_matmul_flops + pv_matmul_flops
    return {
        "mean_microseconds": ms * 1e3,
...
def run_benchmark(configs: list[tuple]):
    ...
    for config in configs:
        ...
        if dtype == "f8E4M3FNUZ":
            triton_dtype = "fp8"
        result = bench_flash_attention(1, B, K2, N, False, triton_dtype)
        result.update({
            "index": index,
            "tag": tag,
            "name": name,
...
def main():
    ...
    output_csv = "results/triton_attention.csv"
