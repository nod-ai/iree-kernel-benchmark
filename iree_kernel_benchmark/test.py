...
all_problems += [
    {
        ...
        'allowedBackends': ['iree', 'torch'] + (['wave'] if problem.tA + problem.tB == 'NT' else []),
        'problem': asdict(problem), 
    } 
...
all_problems += [
    {
        ...
        'tag': tag,
        'dtype': problem.dtype,
        'allowedBackends': ['iree'] + (['torch', 'wave'] if problem.dtype == 'f16' else []),
...
print(f'Generated {len(all_problems)} configs')

with open('configs.json', 'w') as file:
    json.dump(all_problems, file)