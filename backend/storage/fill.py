...
kernel_collections, blob_names = save_all_artifact_kernels(
    repo, directory_client, limit=num_success
)
    ...
    if status in ["completed", "in_progress"]:
        ...
        for i in range(min(num_steps_success + 1, num_total_steps)):
            ...
            if i < num_steps_success:
                step_obj.update({"conclusion": "success", "status": "completed"})
                ...
                step_obj.update(
                    {
                        "conclusion": (
                            "null"
                            if workflow_conclusion == "null"
                            else random.choice(job_conclusions)
                        ),
                        "status": (
                            random.choice(("in_progress", "queued"))
                            if workflow_conclusion == "null"
                            else "completed"
                        ),
                    }
