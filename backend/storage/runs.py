...
from storage.artifacts import load_artifact_kernels, save_run_artifact, compare_artifact_kernels
...
def update_runs(repo: Repository.Repository, db_client: DatabaseClient, dir_client: DirectoryClient):
    print('Updating runs')
    ...
    for stored_run in stored_incomplete_runs:
        ...
        for gh_job in gh_jobs:
            job_steps = [{
                ...
                "number": step.number,
                "started_at": datetime.isoformat(step.started_at) if step.started_at else None,
                "completed_at": datetime.isoformat(step.completed_at) if step.completed_at else None,
            } for step in gh_job.steps]

            print(f'Updating run_{stored_run._id} in db')
            db_client.update_run(stored_run._id, {
...
def update_artifacts(repo: Repository.Repository, db_client: DatabaseClient, dir_client: DirectoryClient):
    ...
    for completed_run in completed_runs:
        ...
        if save_success:
            db_client.update_run(completed_run._id, {
                'hasArtifact': True,
            })

def update_change_stats(repo: Repository.Repository, db_client: DatabaseClient, dir_client: DirectoryClient):
    print('Updating change statistics')
    statless_runs = db_client.query_runs(f"status eq 'completed' and hasArtifact eq true and changeStats eq '{json.dumps({})}'")
    ...
    for statless_run in statless_runs:
        ...
        baseline_kernels = load_artifact_kernels(dir_client, "baseline/benchmark-results")
