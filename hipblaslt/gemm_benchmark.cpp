...
hipDataType parseDatatype(const std::string& dtype_str) {
    if (dtype_str == "fp16" || dtype_str == "f16") return HIP_R_16F;
    if (dtype_str == "bf16") return HIP_R_16BF;
    if (dtype_str == "fp32" || dtype_str == "f32") return HIP_R_32F;
    if (dtype_str == "fp64" || dtype_str == "f64") return HIP_R_64F;
    if (dtype_str == "int8" || dtype_str == "i8") return HIP_R_8I;
    if (dtype_str == "int32" || dtype_str == "i32") return HIP_R_32I;
...
std::string datatypeToString(hipDataType dtype) {
    switch(dtype) {
        case HIP_R_16F: return "fp16";
        case HIP_R_16BF: return "bf16";
        case HIP_R_32F: return "fp32";
        case HIP_R_64F: return "fp64";
        case HIP_R_8I: return "int8";
        case HIP_R_32I: return "int32";
...
void benchmarkGemm(const GemmParams& params, int warmup_runs = 10, int benchmark_runs = 100) {
    ...
    float avg_time_us = 0;
    for (float t : times_us) avg_time_us += t;
    ...
    switch(params.compute_type) {
        case HIPBLAS_COMPUTE_16F: std::cout << "FP16"; break;
        case HIPBLAS_COMPUTE_32F: std::cout << "FP32"; break;
        case HIPBLAS_COMPUTE_64F: std::cout << "FP64"; break;
        case HIPBLAS_COMPUTE_32I: std::cout << "INT32"; break;
