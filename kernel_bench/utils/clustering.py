...
def example_usage_with_mixed_types():
    """Demonstrate usage with both numerical and categorical features."""

    # Example GEMM kernel configurations with mixed types
    # (M, N, K, transpose_variant, datatype)
    kernel_configs = [
        (8096, 412, 367892, "NT", "f16"),
        (8096, 414, 367892, "NT", "f16"),
        (8096, 416, 367892, "NT", "f16"),
        (1024, 512, 2048, "NN", "bf16"),
        (1024, 514, 2048, "NN", "bf16"),
        (2048, 1024, 4096, "TN", "f16"),
        (2048, 1026, 4096, "TN", "f16"),
        (512, 256, 1024, "NT", "f8"),
        (512, 258, 1024, "NT", "f8"),
        (4096, 2048, 8192, "NN", "bf16"),
    ]

    # Convert to DataFrame with meaningful column names
    df = pd.DataFrame(kernel_configs, columns=["M", "N", "K", "transpose", "dtype"])
    ...
    for i, rep in enumerate(representatives):
        print(
            f"  Cluster {i}: M={rep[0]}, N={rep[1]}, K={rep[2]}, "
            f"transpose={rep[3]}, dtype={rep[4]}"
        )

    # Get detailed cluster information
    print("\nCluster Information:")
    print(clusterer.get_cluster_info())

    # Predict cluster for new configuration
    new_config = pd.DataFrame(
        [(8096, 415, 367892, "NT", "f16")],
        columns=["M", "N", "K", "transpose", "dtype"],
    )
    predicted_cluster = clusterer.predict_cluster(new_config)
    print(f"\nNew configuration belongs to cluster: {predicted_cluster[0]}")

    return clusterer, representatives


if __name__ == "__main__":
    # Run example with mixed types
    clusterer, representatives = example_usage_with_mixed_types()
