...
def create_bshd_attention_plot(
    ...
    def hash_result(result: BenchmarkResult) -> str:
        shape = result.shape
        return (
            f'{shape["B"]}x{shape["N_Q"]}x{shape["H"]}x{shape["H_KV"]}x{shape["D_Q"]}'
        )
    ...
    # Customize the plot
    ax.set_xlabel(f"Problem Configuration (BxSxH_QxH_KVxD)", fontsize=14)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Backend to run kernels (eg: wave, torch, triton, all, etc.)\n",
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Comma-separated list of BSHD attention problems (1x1024x32x8x128,1x2048x32x8x128,...)\n"
            "Format: BxSxH_QxH_KVxD where S maps to both N_Q and N_KV, D maps to both D_Q and D_KV"
        ),
    )
    parser.add_argument(
        "--validate",
    ...
    try:
        ...
        configs = [
            (
                ...
                AttentionConfigBSHD(
                    B=problem[0],
                    N_Q=problem[1],  # S maps to N_Q
                    H=problem[2],  # H_Q
                    H_KV=problem[3],  # H_KV
                    D_Q=problem[4],  # D maps to D_Q
                    D_KV=problem[4],  # D maps to D_KV
                    N_KV=problem[1],  # S maps to N_KV
                    dtype=dtype,
                ),
            )
    ...
    for backend_name in backend_names:
        if backend_name not in BENCHMARKS["bshd_attention"]:
            logger.error(
                f"Backend {backend_name} is currently unsupported for BSHD Attention. Exiting..."
            )
            continue

        logger.info(f"Running BSHD Attention benchmarks for backend {backend_name}")

        bench = BenchmarkRunner(
            backend=backend_name,
            kernel_type="bshd_attention",
            device=device,
        ...
        logger.info(
            f"Generated {len(bench.configs)} BSHD Attention configs for backend {backend_name}."
        )

        results.extend(bench.benchmark_kernels(validate_numerics=args.validate))

    create_bshd_attention_plot(results, args.plot, backend_names, problem_shapes)
