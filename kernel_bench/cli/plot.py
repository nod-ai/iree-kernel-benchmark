...
def main():
    ...
    for backend in backend_names:
        results = load_results(args.kernel_type, backend)
        if not results:
            logger.warning(
                f"No results found for backend '{backend}' and kernel type '{args.kernel_type}'"
            )
            continue

        # Filter by machine
        filtered_results = filter_results_by_machine(results, args.machine)
        if not filtered_results:
            logger.warning(
                f"No results found for backend '{backend}' on machine '{args.machine}'"
            )
            continue

        backend_results[backend] = filtered_results
        logger.info(
            f"Loaded {len(filtered_results)} results for backend '{backend}' on machine '{args.machine}'"
        )
